...
public class UpdateAltarScreen extends HandledScreen<UpdateAltarScreenHandler> {
    private static final Identifier SCROLLER_TEXTURE = Identifier.of("pupdates","container/update_altar/ua_scroller");
    private static final Identifier SCROLLER_DISABLED_TEXTURE = Identifier.of("pupdates","container/update_altar/ua_scroller_disabled");
    private static final Identifier RECIPE_SELECTED_TEXTURE = Identifier.of("pupdates","container/update_altar/ua_recipe_selected");
    private static final Identifier RECIPE_HIGHLIGHTED_TEXTURE = Identifier.of("pupdates","container/update_altar/ua_recipe_highlighted");
    private static final Identifier RECIPE_TEXTURE = Identifier.of("pupdates","container/update_altar/ua_recipe");
    private static final Identifier TEXTURE = Identifier.of("pupdates","textures/gui/container/update_altar.png");
    ...
    public UpdateAltarScreen(UpdateAltarScreenHandler handler, PlayerInventory inventory, Text title) {
    ...
    protected void drawMouseoverTooltip(DrawContext context, int x, int y) {
        ...
        if (this.canCraft) {
            ...
            List<RecipeEntry<UpdateAltarRecipe>> list = this.handler.getAvailableRecipes();
            ...
            for (int l = this.scrollOffset; l < k && l < this.handler.getAvailableRecipeCount(); l++) {
                ...
                if (x >= n && x < n + 16 && y >= o && y < o + 18) {
                    context.drawItemTooltip(
                            this.textRenderer, ((UpdateAltarRecipe)((RecipeEntry)list.get(l)).value()).getResult(this.client.world.getRegistryManager()), x, y
                    );
                }
            }
        }
    ...
    private void renderRecipeIcons(DrawContext context, int x, int y, int scrollOffset) {
        ...
        for (int i = this.scrollOffset; i < scrollOffset && i < this.handler.getAvailableRecipeCount(); i++) {
            ...
            int l = j / 4;
            int m = y + l * 18 + 2;
            context.drawItem(((UpdateAltarRecipe)((RecipeEntry)list.get(i)).value()).getResult(this.client.world.getRegistryManager()), k, m);
        }
    }
